...
{
    internal class WaterBalloonComet : Comet
    {
        ...
        { 
            ...
            SimMessages.EmitMass(cell, ElementLoader.GetElementIndex(SimHashes.Water), Config.Instance.WaterBalloonContentMass, 318.15f, 0, 0);

            //Debug.Log("Chance For Fish" + Config.Instance.ChanceForFish);
            float rand = UnityEngine.Random.Range(0f, 100f);
            //Debug.Log(rand);
            if ( rand < Config.Instance.ChanceForFish)
            {
    ...
    public class Comet_Sim33ms_Patch
    {
        public static void Postfix(Comet __instance)
        {
            ...
            if (!__instance.TryGetComponent<WaterBalloonComet>(out var _))
                return;
