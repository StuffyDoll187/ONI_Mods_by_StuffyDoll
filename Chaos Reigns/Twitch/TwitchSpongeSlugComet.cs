...
{
    internal class TwitchSpongeSlugComet : Comet
    {
        ...
        protected override void SpawnCraterPrefabs()
        {
            ...
            animController.onAnimComplete += delegate
            {
                if (craterPrefabs != null && craterPrefabs.Length != 0)
                {
                    ...
                    if (Config.Instance.TwitchSlugContentElementChoice1 != Config.LiquidOptions.None)
                    {
                        var storage = gameObject.GetComponent<Storage>();
                        storage.AddLiquid(Config.Instance.TwitchSlugContentElement1, Config.Instance.TwitchSlugContentMass1, Config.Instance.TwitchSlugContentTemp1 + 273.15f, 0, 0);
                    }
                    if (Config.Instance.TwitchSlugContentElementChoice2 != Config.LiquidOptions.None)
                    {
                        var storage = gameObject.GetComponent<Storage>();
                        storage.AddLiquid(Config.Instance.TwitchSlugContentElement2, Config.Instance.TwitchSlugContentMass2, Config.Instance.TwitchSlugContentTemp2 + 273.15f, 0, 0);
                    }
                    if (Config.Instance.TwitchSlugContentElementChoice3 != Config.LiquidOptions.None)
                    {
                        var storage = gameObject.GetComponent<Storage>();
                        storage.AddLiquid(Config.Instance.TwitchSlugContentElement3, Config.Instance.TwitchSlugContentMass3, Config.Instance.TwitchSlugContentTemp3 + 273.15f, 0, 0);
                    }

                    var agemonitor = gameObject.GetSMI<AgeMonitor.Instance>();
                    
                    agemonitor.age.value = agemonitor.age.GetMax() - (Config.Instance.TwitchSlugTTL / 600f);//* 0.99995f;
                }
