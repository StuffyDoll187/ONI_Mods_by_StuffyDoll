...
{
    internal class ZoologicalComet : Comet
    {       
        ...
        protected override void SpawnCraterPrefabs()
        {
            ...
            animController.onAnimComplete += delegate
            {
                if (critters != null && critters.Count != 0)
                {
                    ...
                    GameObject gameObject = Util.KInstantiate(Assets.GetPrefab(ZoologicalComet.critters[UnityEngine.Random.Range(0, ZoologicalComet.critters.Count)]), position);
