...
namespace DuplicantLifecycles
{
    public static class DuplicantLifecycleConfigChecker
    {
        ...
        public static bool EnableDeath { get { return enableDeath; } }
        private static bool enableImmortalTrait;
        public static bool EnableImmortalTrait { get { return enableImmortalTrait; } }
        ...
        internal static void CheckConfigVariables()
        {
            ...
            bool temp_enableDeath = true;
            bool temp_enableImmortalTrait = true;
            ...
            if (!config.EnabledDeathFromAge)
                temp_enableDeath = false;

            if (temp_enableDeath && AgeSanityCheck(config.MaxDuplicantAge))
                temp_maxAge = config.MaxDuplicantAge;
            else if (temp_enableDeath)
                LogManager.LogWarning("Config file argument (MaxDuplicantAge) was invalid." +
                        "\nValue was not a positive floating point number: " + config.MaxDuplicantAge);
            else
                LogManager.Log("Config file argument (MaxDuplicantAge) was ignored." +
                    "\nEnableDeathFromAge is false therefore unused argument: " + config.MaxDuplicantAge);

            if (AgeSanityCheck(config.YouthfulAgeCutoff))
                temp_youthCutoff = config.YouthfulAgeCutoff;
            else
                LogManager.LogWarning("Config file argument (YouthfulAgeCutoff) was invalid." +
                        "\nValue was not a positive floating point number: " + config.YouthfulAgeCutoff);

            if (AgeSanityCheck(config.MiddleAgedAgeCutoff))
                temp_middleCutoff = config.MiddleAgedAgeCutoff;
            else
                LogManager.LogWarning("Config file argument (MiddleAgedAgeCutoff) was invalid." +
                            "\nValue was not a positive floating point number: " + config.MiddleAgedAgeCutoff);

            if (AgeSanityCheck(config.ElderlyAgeCutoff))
                temp_elderlyCutoff = config.ElderlyAgeCutoff;
            else
                LogManager.LogWarning("Config file argument (ElderlyAgeCutoff) was invalid." +
                        "\nValue was not a positive floating point number: " + config.ElderlyAgeCutoff);
            ...
            if (!temp_usePercentage && MultiplierSanityCheck(config.CustomStatBaseValue))
                ...
                LogManager.LogWarning("Config file argument (CustomStatBaseValue) was invalid." +
                        "\nValue was not a floating point number between -10 and 10: " + config.CustomStatBaseValue);
            else
                LogManager.Log("Config file argument (CustomStatBaseValue) was ignored." +
                    "\nUsePercentageOfTotalStats is true therefore unused argument: " + config.CustomStatBaseValue);

                
            if (AgeSanityCheck(config.TimeToDieProbabilityDecrease))
                temp_probabilityDecrease = config.TimeToDieProbabilityDecrease;
            else
                LogManager.LogWarning("Config file argument (TimeToDieProbabilityIncrease) was invalid." +
                        "\nValue was not a positive floating point number: " + config.TimeToDieProbabilityDecrease);

            if (!config.EnableImmortalTrait)
                temp_enableImmortalTrait = false;


            if (MultiplierSanityCheck(config.YouthAttributeMultiplier))
                temp_youthMultiplier = config.YouthAttributeMultiplier;
            else
                LogManager.LogWarning("Config file argument (YouthAttributeMultiplier) was invalid." +
            ...
            if (MultiplierSanityCheck(config.MiddleAgedAttributeMultiplier))
                ...
                LogManager.LogWarning("Config file argument (MiddleAgedAttributeMultiplier) was invalid." +
            ...
            if (MultiplierSanityCheck(config.ElderlyAttributeMultiplier))
                ...
                LogManager.LogWarning("Config file argument (ElderlyAttributeMultiplier) was invalid." +
            ...
            if (MultiplierSanityCheck(config.DyingAttributeMultiplier))
                ...
                LogManager.LogWarning("Config file argument (DyingAttributeMultiplier) was invalid." +
                        "\nValue was not a floating point number between -10 and 10: " + config.DyingAttributeMultiplier);

            if (temp_enableImmortalTrait && MultiplierSanityCheck(config.ImmortalAttributeMultiplier))
                temp_immortalMultiplier = config.ImmortalAttributeMultiplier;
            else if (temp_enableImmortalTrait)
                LogManager.LogWarning("Config file argument (ImmortalAttributeMultiplier) was invalid." +
                        "\nValue was not a floating point number between -10 and 10: " + config.ImmortalAttributeMultiplier);
            else
                LogManager.Log("Config file argument (ImmortalAttributeMultiplier) was ignored." +
                    "\n EnableImmortalTrait is false therefore unused argument: " + config.ImmortalAttributeMultiplier);
            ...
            if (temp_middleCutoff <= temp_youthCutoff)
            {
                ...
                LogManager.LogWarning("Config file argument (MiddleAgedAgeCutoff) was less than YouthfulAgeCutoff: " +
            ...
            {
                useDefaultCutoffs = true;
                LogManager.LogWarning("Config file argument (ElderlyAgeCutoff) was less than MiddleAgedAgeCutoff: " +
            ...
            {
                useDefaultCutoffs = true;
                LogManager.LogWarning("Config file argument (MaxDuplicantAge) was less than ElderlyAgeCutoff: " +
            ...
            DuplicantLifecycleConfigChecker.enableDeath = temp_enableDeath;
            DuplicantLifecycleConfigChecker.enableImmortalTrait = temp_enableImmortalTrait;
