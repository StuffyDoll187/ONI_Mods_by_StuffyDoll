...
{
    public class AgingTrait : StateMachineComponent<AgingTrait.StatesInstance>
    {
        ...
        {            
            if (gameObject.GetComponent<MinionIdentity>().arrivalTime <= 0)
            {
        ...
        private List<AttributeModifier> CalculateAgeModifiers(float multiplier)
        {
            ...
            var ageModifierList = new List<AttributeModifier>()
            {
            ...
            if (DlcManager.IsContentSubscribed("EXPANSION1_ID"))
                ageModifierList.Add(new AttributeModifier("SpaceNavigation", this.GetModifierDelta("SpaceNavigation", multiplier), (string)DuplicantLifecycleStrings.AgingNAME, false, false, false));            
            return ageModifierList;
        ...
        private float GetModifierDelta(string id, float multiplier)
        {
            ...
            {
                //AttributeLevels attributeLevels = this.gameObject.GetComponent<AttributeLevels>();
                //int level = attributeLevels.GetAttributeLevel(id).level;
                ...
                else if (i.GetTotalValue() > 0)// && i.GetTotalValue() > level)
                    base_value = i.GetTotalValue();
                else
                    base_value = 0;
                    //base_value = level;
                //Debug.Log(i.name.ToString() + " " + i.Attribute.Id + " "+ i.GetTotalValue());
        ...
        {
            // Cryo Dupes subtract 1000-2000 from arrival time
            // Jorge subtracts 2190-3102
        ...
        protected bool IsImmortal()
        {
            ...
            //bool flag = traits.HasTrait(DuplicantLifecycleStrings.ImmortalID) || traits.HasTrait("AncientKnowledge");
            return this.GetComponent<Traits>().HasTrait(DuplicantLifecycleStrings.ImmortalID);
        ...
        protected void RefreshModifiers(StatesInstance smi, float dt) 
        {
            /*refreshDelay += dt;
            if (refreshDelay <= 30)
                return;
            ...
            if (state == smi.sm.youthful)
            {
                RemoveModifiers(youthAttributeModifiers);
                SetupModifierLists();
                ApplyYouthfulModifiers();
                return;
            }
            if (state == smi.sm.middleaged)
            { 
                RemoveModifiers(middleAttributeModifiers);
                SetupModifierLists();
                ApplyMiddleAgedModifiers(); 
                return;
            }
            if (state == smi.sm.elderly)
            {
                RemoveModifiers(elderlyAttributeModifiers);
                SetupModifierLists();
                ApplyElderlyModifiers();                
                return;
            }
            if (state == smi.sm.dying)
            { 
                RemoveModifiers(dyingAttributeModifiers);
                SetupModifierLists();
                ApplyDyingModifiers(); 
                return;
            }
            if (state == smi.sm.immortal)
            {
                RemoveModifiers(immortalAttributeModifiers);
                SetupModifierLists();
                ApplyImmortalModifiers();
                return;
            }
        }
        ...
        public class StatesInstance : GameStateMachine<AgingTrait.States, AgingTrait.StatesInstance, AgingTrait, object>.GameInstance
        {
            public StatesInstance(AgingTrait master) : base(master) { }
            
        }

        public class States : GameStateMachine<AgingTrait.States, AgingTrait.StatesInstance, AgingTrait>
        {
            public GameStateMachine<AgingTrait.States, AgingTrait.StatesInstance, AgingTrait, object>.State noage;
            public GameStateMachine<AgingTrait.States, AgingTrait.StatesInstance, AgingTrait, object>.State immortal;
            public GameStateMachine<AgingTrait.States, AgingTrait.StatesInstance, AgingTrait, object>.State youthful;
            public GameStateMachine<AgingTrait.States, AgingTrait.StatesInstance, AgingTrait, object>.State middleaged;
            public GameStateMachine<AgingTrait.States, AgingTrait.StatesInstance, AgingTrait, object>.State elderly;
            public GameStateMachine<AgingTrait.States, AgingTrait.StatesInstance, AgingTrait, object>.State dying;
            public GameStateMachine<AgingTrait.States, AgingTrait.StatesInstance, AgingTrait, object>.State dead;

            public override void InitializeStates(out StateMachine.BaseState default_state)
            {
                default_state = this.noage;
                this.noage
                    .Transition(this.immortal, (smi => smi.master.IsImmortal()), UpdateRate.SIM_200ms)
                    .Transition(this.youthful, (smi => smi.master.IsYouthful()), UpdateRate.SIM_200ms)
                    .Transition(this.middleaged, (smi => !smi.master.IsYouthful() && !smi.master.IsElderly() && !smi.master.IsDying()), UpdateRate.SIM_200ms)
                    .Transition(this.elderly, (smi => smi.master.IsElderly()), UpdateRate.SIM_200ms)
                    .Transition(this.dying, (smi => smi.master.IsDying()), UpdateRate.SIM_200ms)
                    .Transition(this.dead, (smi => smi.master.IsDead()), UpdateRate.SIM_200ms);
                this.immortal
                    .Enter("Immortal", (smi => smi.master.ApplyImmortalModifiers()))
                    .Exit("NotImmortal", (smi => smi.master.RemoveModifiers(smi.master.immortalAttributeModifiers)))
                    .ToggleStatusItem(DuplicantLifecycleStrings.Immortal, null)
                    .ToggleExpression(Db.Get().Expressions.Relief, null)
                    .TagTransition(GameTags.Dead, this.dead)
                    .Update((smi, dt) => smi.master.RefreshModifiers(smi, dt), UpdateRate.SIM_4000ms);
                this.youthful
                    .Enter("Youthful", (smi => smi.master.ApplyYouthfulModifiers()))
                    .Exit("NotYouthful", (smi => smi.master.RemoveModifiers(smi.master.youthAttributeModifiers)))
                    .ToggleStatusItem(DuplicantLifecycleStrings.AgingYouth, null)
                    .ToggleExpression(Db.Get().Expressions.Happy, null)
                    .Transition(this.middleaged, (smi => smi.master.IsMiddleAged()), UpdateRate.SIM_4000ms)
                    .Transition(this.immortal, (smi => smi.master.IsImmortal()), UpdateRate.SIM_1000ms)
                    .TagTransition(GameTags.Dead, this.dead)
                    .Update((smi,dt) => smi.master.RefreshModifiers(smi, dt), UpdateRate.SIM_4000ms);

                this.middleaged
                    .Enter("MiddleAged", (smi => smi.master.ApplyMiddleAgedModifiers()))
                    .Exit("NotMiddleAged", (smi => smi.master.RemoveModifiers(smi.master.middleAttributeModifiers)))
                    .ToggleStatusItem(DuplicantLifecycleStrings.AgingMiddle, null)
                    .Transition(this.elderly, (smi => smi.master.IsElderly()), UpdateRate.SIM_4000ms)
                    .Transition(this.immortal, (smi => smi.master.IsImmortal()), UpdateRate.SIM_1000ms)
                    .TagTransition(GameTags.Dead, this.dead)
                    .Update((smi, dt) => smi.master.RefreshModifiers(smi, dt), UpdateRate.SIM_4000ms); 
                this.elderly
                    .Enter("Elderly", (smi => smi.master.ApplyElderlyModifiers()))
                    .Exit("NotElderly", (smi => smi.master.RemoveModifiers(smi.master.elderlyAttributeModifiers)))
                    .ToggleStatusItem(DuplicantLifecycleStrings.AgingElderly, null)
                    .ToggleExpression(Db.Get().Expressions.Tired, null)
                    .Transition(this.dying, (smi => smi.master.IsDying()), UpdateRate.SIM_4000ms)
                    .Transition(this.immortal, (smi => smi.master.IsImmortal()), UpdateRate.SIM_1000ms)
                    .TagTransition(GameTags.Dead, this.dead)
                    .Update((smi, dt) => smi.master.RefreshModifiers(smi, dt), UpdateRate.SIM_4000ms); 
                this.dying
                    .Enter("Dying", (smi => smi.master.ApplyDyingModifiers()))
                    .ToggleStatusItem(DuplicantLifecycleStrings.AgingDying, null)
                    .ToggleExpression(Db.Get().Expressions.Sick, null)
                    .Transition(this.dead, (smi => smi.master.TimeToDie()), UpdateRate.SIM_4000ms)
                    .Transition(this.immortal, (smi => smi.master.IsImmortal()), UpdateRate.SIM_1000ms)
                    .TagTransition(GameTags.Dead, this.dead)
                    .Update((smi, dt) => smi.master.RefreshModifiers(smi, dt), UpdateRate.SIM_4000ms); 
                this.dead
                    .Enter("Dead", (smi => smi.StopSM("Death")));
            }
        }
    }
}